# Modified from https://github.com/jzliu-100/visualize-neural-network
# which in turn is modified based the work by Milo Spencer-Harper and Oli Blum,
# https://stackoverflow.com/a/37366154/10404826
...
class Neuron:
    ...
    def draw(self, ax, neuron_radius, id=-1):
...
class Layer:
    def __init__(
        self, network, number_of_neurons, number_of_neurons_in_widest_layer
    ):
    ...
    def __calculate_layer_y_position(self):
        if self.previous_layer:
            return (
                self.previous_layer.y + self.vertical_distance_between_layers
            )
        else:
            return 0
    ...
    def __line_between_two_neurons(
        self, ax, neuron1, neuron2, weight=0.4, textoverlaphandler=None
    ):
    ...
    def draw(self, ax, layerType=0, weights=None, textoverlaphandler=None):
...
class TextOverlappingHandler:
    ...
    def __init__(self, width, height, grid_size=0.2):
        ...
        self.cells = np.ones(
            (
                int(np.ceil(width / grid_size)),
                int(np.ceil(height / grid_size)),
            ),
            dtype=bool,
        )

    # input test_coordinates(bottom left and top right), getspace will tell you
    # whether a text label can be put in the test coordinates
...
class NeuralNetwork:
    ...
    def draw(self, weights_list=None, figsize=(12, 9)):
        # vertical_distance_between_layers and
        # horizontal_distance_between_neurons are the same with the variables
        # of the same name in layer class
...
class DrawNN:
    # para: neural_network is an array of the number of neurons from input
    # layer to output layer, e.g., a neural network of 5 nerons in the input
    # layer, 10 neurons in the hidden layer 1 and 1 neuron in the output layer
    # is [5, 10, 1] para: weights_list (optional) is the output weights list of
    # a neural network which can be obtained via classifier.coefs_
    def __init__(self, neural_network, weights_list=None):
        self.neural_network = neural_network
        self.weights_list = weights_list
        # if weights_list is none, then create a uniform list to fill the
        # weights_list
